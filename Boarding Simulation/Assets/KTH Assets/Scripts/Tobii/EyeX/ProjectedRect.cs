...
public struct ProjectedRect
{
    ...
    /// <summary>
    /// Gets the projection for a given game object and camera.
    /// </summary>
    /// <param name="gameObject">The game object.</param>
    /// <param name="camera">The camera.</param>
    /// <returns>The projection.</returns>
    public static ProjectedRect GetProjectedRect(GameObject gameObject, Camera camera)
    {
        float xMin = 0, xMax = 0, yMin = 0, yMax = 0, zMin = 0, zMax = 0;

        bool first = true;
        foreach (var point in GetBoundingBoxCornerPoints(gameObject.GetComponent<Renderer>().bounds))
        {
